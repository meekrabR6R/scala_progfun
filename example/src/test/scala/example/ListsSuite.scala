...
class ListsSuite extends FunSuite {
  ...
  test("one plus four is five")(assert(1 + 4 === 5))
  /**
  ...
  test("sum of a negative and positive number") {
  ...
  test("max of list with duplicate number") {
    assert(max(List(1, 2, 2, 4)) === 4)
  }

  test("max of list of negative numbers") {
